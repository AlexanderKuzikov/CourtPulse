...
| 2026-08-22 | **Чистый старт**: замусоренные данные 12-22.08 (`TIMEOUT` баг, 301, arbitr, `concurrency 4` бан) обнулены — `stop → tar czf /tmp/courtpulse_backup_20260822.tar.gz (876К) → rm probes/*.jsonl state/incidents` на `sat`+локально, `restart 15:56 UTC` (`UNKNOWN 183` → первая волна ~16:05) |
